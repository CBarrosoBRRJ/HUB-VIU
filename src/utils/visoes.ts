...
import { AppPage, Equipe, Usuario } from '../types';
...
export interface Visao {
  ...
  quadro: AppPage;
  label: string;
  /** Exige liberação explícita à equipe. Sem ela, só o dono e o admin enxergam. */
  restrita?: boolean;
  /** Por que é restrita — exibido na tela de Equipes, ao lado do chip. */
  motivo?: string;
}
...
export function visoesDoQuadro(quadro: AppPage): Visao[] {
  return VISOES.filter((visao) => visao.quadro === quadro);
}

export function getVisao(id: string): Visao | undefined {
  return VISOES.find((visao) => visao.id === id);
}

/** Visões restritas — as únicas que aparecem para liberar na tela de Equipes. */
export function visoesRestritasDoQuadro(quadro: AppPage): Visao[] {
 ...
 */
export function podeVerVisao(
  { usuario, equipes }: ContextoVisao,
  visaoId: string,
  ehDono = false,
): boolean {
  ...
  if (visao.restrita) return minhas.some((e) => (e.visoesLiberadas ?? []).includes(visaoId));
...
export function alternarVisao(equipe: Equipe, visaoId: string): Equipe {
 ...
 * abas sensíveis.
 */
export function alternarVisaoOculta(equipe: Equipe, visaoId: string): Equipe {
...
export function colunasOcultasNaVisao(
  contexto: ContextoVisao,
  colunasDaVisao: { id: string }[],
  ehDono = false,
): string[] {
...
export function alternarColuna(equipe: Equipe, colunaId: string): Equipe {
