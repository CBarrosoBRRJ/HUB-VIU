...
export function somarDiasUteis(iso: string, dias: number): string {
  ...
  while (restantes > 0) {
    ...
    if (!ehFimDeSemana(data)) restantes -= 1;
  }
...
export function prazoDeTriagem(entradaISO: string): string {
  return somarDiasUteis(entradaISO, SLA_DIAS_UTEIS);
}

export type SlaTone = 'verde' | 'amarelo' | 'vermelho' | 'cinza';

export interface SlaInfo {
  tone: SlaTone;
  label: string;
  /** Dias úteis restantes; negativo quando estourou. `null` quando não há prazo a medir. */
  diasRestantes: number | null;
  /** Percentual do prazo consumido (0–100). `null` sem prazo apurável. */
 ...
 */
export const SLA_TONE_STYLE: Record<
  SlaTone,
  { text: string; bar: string; dot: string; faixa: string }
> = {
  /*
...
interface AlvoSla {
  entradaEm: string;
  prazoEm: string;
  /** Já saiu da triagem? Fecha o relógio. */
  encerrada: boolean;
}
 ...
 */
export function getSlaInfo(alvo: AlvoSla, referencia = new Date()): SlaInfo {
  if (alvo.encerrada) {
