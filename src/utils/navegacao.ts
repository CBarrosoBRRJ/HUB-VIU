...
export const PAGINAS_APP: AppPage[] = [
  'backlog',
  'contratos',
  'talentos',
  'clientes',
...
export const PAGINA_PADRAO: AppPage = 'contratos';
...
export const CAMINHOS: Record<AppPage, string> = {
  backlog: '/backlog',
  contratos: '/contratos',
  talentos: '/talentos',
  clientes: '/clientes',
 ...
 */
export function paginaPorCaminho(pathname: string): AppPage | null {
  const limpo = `/${(pathname ?? '').toLowerCase().split('/').filter(Boolean).join('/')}`;
  const achada = (Object.entries(CAMINHOS) as [AppPage, string][])
    .find(([, caminho]) => caminho === limpo);
  return achada ? achada[0] : null;
}

export function caminhoDaPagina(pagina: AppPage): string {
 ...
 */
export function validarPagina(valor: unknown, padrao: AppPage = PAGINA_PADRAO): AppPage {
  return typeof valor === 'string' && PAGINAS_APP.includes(valor as AppPage)
    ? (valor as AppPage)
    : padrao;
}
