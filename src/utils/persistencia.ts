...
export function carregar<T>(nome: string, padrao: T): T {
  try {
    ...
    return Array.isArray(valor) ? (semIdsRepetidos(valor) as T) : (valor as T);
  } catch {
    return padrao;
  }
}
 ...
 */
function semIdsRepetidos(lista: unknown[]): unknown[] {
...
export function salvar(nome: string, valor: unknown): void {
  try {
...
export function limparTudo(): void {
  try {
    ...
    for (let i = 0; i < window.localStorage.length; i += 1) {
      ...
      if (nome?.startsWith(`${PREFIXO}:`)) alvos.push(nome);
    }
...
export function temDadosSalvos(): boolean {
  try {
