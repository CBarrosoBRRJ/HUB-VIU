...
export function distancia(a, b) {
    ...
    let atual = new Array(y.length + 1);
...
export function similaridade(a, b) {
    ...
    const soma = menor.reduce((total, token) => total + Math.max(...maior.map((outro) => casarToken(token, outro))), 0);
 ...
 */
export function semelhantes(valor, opcoes, limite = 3) {
    const alvo = normalizar(valor);
    if (!alvo)
        return [];
    if (opcoes.some((opcao) => normalizar(opcao) === alvo))
        return [];
    return opcoes
 ...
 */
export function sugestoes(termo, opcoes, limite = 6) {
    ...
    for (const opcao of unicas) {
        ...
        if (normalizada.startsWith(alvo))
            comeca.push(opcao);
        else if (normalizada.includes(alvo))
            contem.push(opcao);
    }
 ...
 */
export function valoresUsados(registros, campo) {
    ...
    for (const registro of registros) {
        ...
        const variantes = grupos.get(chave) ?? new Map();
 ...
 */
export function divergencias(registros, campo) {
    const grupos = new Map();
    for (const registro of registros) {
        const bruto = registro[campo];
        if (typeof bruto !== 'string')
        ...
        if (!valor)
            continue;
        const chave = normalizar(valor);
        const set = grupos.get(chave) ?? new Set();
        set.add(valor);
