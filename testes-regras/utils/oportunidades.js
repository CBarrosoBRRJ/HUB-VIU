 ...
 */
export function saiuDaTriagem(oportunidade) {
    return !getStatus(oportunidade.status).emTriagem;
}
export function slaDaOportunidade(oportunidade, referencia = new Date()) {
    return getSlaInfo({
 ...
 */
export const DESFECHOS = ['fechado', 'standby', 'declinado', 'encerrado'];
/** Os que realmente encerram — sem volta pelo fluxo. */
export const DESFECHOS_TERMINAIS = DESFECHOS.filter((status) => getStatus(status).encerra);
/**
...
export function finalizadaRecentemente(oportunidade, referencia = new Date()) {
    if (!getStatus(oportunidade.status).encerra)
        return false;
    const desde = parseData(oportunidade.statusDesde);
    if (!desde)
        return true; // Sem data, é melhor mostrar do que sumir com o registro.
 ...
 */
export function emAndamento(oportunidades) {
...
export function finalizadasAntigas(oportunidades, referencia = new Date()) {
    return oportunidades.filter((op) => getStatus(op.status).encerra && !finalizadaRecentemente(op, referencia)).length;
}
/**
 ...
 * que ninguém confia.
 */
export function resumirPorStatus(oportunidades, status, referencia = new Date()) {
    const encerra = Boolean(getStatus(status).encerra);
    const doStatus = oportunidades.filter((op) => op.status === status && (!encerra || finalizadaRecentemente(op, referencia)));
...
export function getMotivoDeclinio(id) {
 ...
 */
export function rotuloDoStatus(status, motivoId, curto = false) {
 ...
 */
export function getPrioridade(id) {
...
export function getEntradaPor(id) {
...
export function getOrigemComercial(id) {
...
export function nomeadosDaOportunidade(oportunidade) {
    /*
    ...
    */
    return [
        ...new Set([
            ...Object.values(oportunidade.responsaveis).flat().filter(Boolean),
            ...Object.values(oportunidade.apoios ?? {}).flat().filter(Boolean),
        ]),
    ];
}
export function responsaveisDaAreaNaOportunidade(oportunidade, area) {
    return oportunidade.responsaveis[area] ?? [];
}
export function alternarResponsavelNaOportunidade(oportunidade, area, usuarioId) {
    ...
    if (proximos.length > 0)
        responsaveis[area] = proximos;
    else
        delete responsaveis[area];
    return { ...oportunidade, responsaveis };
}
export function matchesFiltroOportunidade(oportunidade, filtro, referencia = new Date()) {
...
export function totaisDoRodape(oportunidades, referencia = new Date()) {
    const porInput = INPUTS.reduce((acc, input) => ({ ...acc, [input.id]: 0 }), {});
    let slaAtrasados = 0;
    let slaAtencao = 0;
    let slaNoPrazo = 0;
    let exclusivos = 0;
    ...
    for (const op of oportunidades) {
        ...
        */
        if (op.input)
            porInput[op.input] = (porInput[op.input] ?? 0) + 1;
        else
            semInput += 1;
    }
    return {
 ...
 */
export function contarOportunidades(oportunidades, referencia = new Date()) {
    const conta = (filtro) => oportunidades.filter((o) => matchesFiltroOportunidade(o, filtro, referencia)).length;
    return {
...
export function compararPorCampo(a, b, campo) {
    ...
    if (escala) {
        ...
        return posicao(a[campo]) - posicao(b[campo]);
    }
    // Exclusivo é booleano: "sim" antes de "não" é mais útil que "false" antes de "true".
    if (campo === 'exclusivo')
        return Number(b.exclusivo) - Number(a.exclusivo);
    return String(a[campo] ?? '')
        .localeCompare(String(b[campo] ?? ''), 'pt-BR');
}
/* ------------------------------------------------------------------ *
 * Exclusividade
 * ------------------------------------------------------------------ */
 ...
 */
export function exclusividadeDe(oportunidade, talentos) {
    if (!oportunidade.talento?.trim())
        return undefined;
    /*
    ...
    */
    const ficha = (oportunidade.talentoId && talentos.find((t) => t.id === oportunidade.talentoId)) ||
        talentos.find((t) => t.nome.trim().toLowerCase() === oportunidade.talento.trim().toLowerCase());
    if (!ficha)
        return undefined;
    return ficha.tipo === 'exclusivo';
}
export function apoiosDaAreaNaOportunidade(oportunidade, area) {
    return oportunidade.apoios?.[area] ?? [];
}
/** O papel que a pessoa ocupa nesta área, ou `null` se não está na linha. */
export function papelNaArea(oportunidade, area, usuarioId) {
    if (responsaveisDaAreaNaOportunidade(oportunidade, area).includes(usuarioId))
        return 'responsavel';
    if (apoiosDaAreaNaOportunidade(oportunidade, area).includes(usuarioId))
        return 'apoio';
    return null;
}
/**
 ...
 */
export function definirPapelNaArea(oportunidade, area, usuarioId, papel) {
    ...
    if (papel === 'responsavel')
        responsaveis.push(usuarioId);
    if (papel === 'apoio')
        apoios.push(usuarioId);
    return {
        ...oportunidade,
...
export function alternarPapelNaArea(oportunidade, area, usuarioId, papel) {
...
export function getTipoOutput(id) {
...
export function getTipoEdicao(id) {
...
export function getFormatoConteudo(id) {
 ...
 */
export const ALCANCES_AUDIENCIA = [
...
export function getAlcanceAudiencia(id) {
...
export function destravaDaColuna(campo) {
    return ESCOPO_DESTRAVA.find((par) => par.campo === campo);
}
/** O par de um tique do Escopo. */
export function destravaDoTique(tique) {
 ...
 */
export function destravada(op, campo) {
 ...
 */
export function comValorSemTique(oportunidades) {
    return oportunidades.flatMap((op) => ESCOPO_DESTRAVA
 ...
 * ------------------------------------------------------------------ */
export const PENDENCIAS_POR_STATUS = {
    elaboracao: [
        {
 ...
 */
export function diasDeEspera(pendencia, hojeISO) {
    const fim = pendencia.chegouEm ?? hojeISO;
    const dias = Math.round((Date.parse(`${fim}T00:00:00`) - Date.parse(`${pendencia.abertaEm}T00:00:00`)) / 86_400_000);
 ...
 */
export function descricaoDasEsperas(op, hojeISO) {
 ...
 */
export function comPendenciaAberta(op, tipo, hojeISO, id) {
    if (pendenciasAbertas(op).some((p) => p.tipo === tipo))
        return op;
    const nova = { id, tipo, statusAbertura: op.status, abertaEm: hojeISO };
    return { ...op, pendencias: [...(op.pendencias ?? []), nova] };
}
/** "✓ Chegou": a resposta veio, o relógio para. */
export function comPendenciaChegada(op, id, hojeISO) {
    return {
        ...op,
        pendencias: (op.pendencias ?? []).map((p) => p.id === id && !p.chegouEm ? { ...p, chegouEm: hojeISO } : p),
    };
}
/** "↩ Reabrir": o "chegou" foi engano (ou a resposta veio incompleta) — o relógio original segue. */
export function comPendenciaReaberta(op, id) {
    return {
        ...op,
        pendencias: (op.pendencias ?? []).map((p) => {
            if (p.id !== id || !p.chegouEm)
                return p;
 ...
 */
export function semPendencia(op, id) {
    return {
        ...op,
 ...
 */
export function bloqueadaPorPendencias(op, destino) {
 ...
 */
export function comPendenciaImpossivel(oportunidades) {
    return oportunidades.flatMap((op) => {
        const problemas = [];
        const abertasPorTipo = new Map();
        (op.pendencias ?? []).forEach((p) => {
            if (p.chegouEm && p.chegouEm < p.abertaEm)
                problemas.push(`${op.id}:${p.id}:chegou-antes`);
        ...
        abertasPorTipo.forEach((quantas, tipo) => {
            if (quantas > 1)
                problemas.push(`${op.id}:${tipo}:aberta-duplicada`);
        });
        return problemas;
    });
}
 ...
 */
export const CAPTACOES_PRODUCAO = [
...
export function getCaptacaoProducao(id) {
