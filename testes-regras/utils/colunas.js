 ...
 */
import { areasDoTalento, REDES } from './talentos.js';
/* ------------------------------------------------------------------ *
 * Quadro: Talentos
 * ------------------------------------------------------------------ */
const T = 'talentos';
export const COLUNAS_TALENTOS = {
    [`${T}:identificacao`]: [
        {
 ...
 */
export const ANCORAS_BACKLOG = ['status', 'entradaEm', 'talento'];
export function ehAncora(coluna) {
    return ANCORAS_BACKLOG.includes(coluna.id.split(':')[2]);
}
/**
 ...
 */
export function larguraDaColuna(coluna) {
 ...
 */
const CONGELADAS_ENXUTAS = ['status'];
export function gradeContinua(visoesVisiveis, ancorasEnxutas = false) {
    ...
    */
    ...
    const sufixo = (coluna) => coluna.id.split(':')[2];
    const congela = (coluna) => ancorasEnxutas
        ? CONGELADAS_ENXUTAS.includes(sufixo(coluna))
        : ehAncora(coluna);
