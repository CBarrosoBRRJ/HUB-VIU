...
const COLECOES = ['oportunidades', 'contratos', 'talentos', 'marcas'];
 ...
 */
export function semMudanca(antes, depois) {
    return COLECOES.every((colecao) => antes[colecao] === depois[colecao]);
}
/** O nome pelo qual a operação chama este registro. */
function rotuloDoItem(colecao, item) {
    const registro = item;
    const candidato = colecao === 'oportunidades'
        ? registro.titulo
        : colecao === 'contratos'
            ? registro.talento
            : registro.nome;
 ...
 */
export function descreverMudanca(antes, depois) {
    for (const colecao of COLECOES) {
        const listaAntes = antes[colecao];
        const listaDepois = depois[colecao];
        if (listaAntes === listaDepois)
            continue;
        if (listaDepois.length > listaAntes.length) {
            ...
            if (novos.length > 1)
                return `Criação de ${contar(novos.length, colecao)}`;
            const novo = novos[0];
            // Duplicar deixa rastro na própria linha — é o que separa "criou" de "copiou" sem adivinhar.
            const duplicado = Boolean(novo?.duplicadaDe);
        ...
        if (listaDepois.length < listaAntes.length) {
            ...
            if (removidos.length > 1)
                return `Exclusão de ${contar(removidos.length, colecao)}`;
        ...
        */
        const conhecidos = new Set(listaAntes);
 ...
 */
export function empilhar(pilha, item, limite = LIMITE_HISTORICO) {
 ...
 */
export function desfazer(passado, futuro, atual) {
    const entrada = passado[passado.length - 1];
    if (!entrada)
        return { instantaneo: null, passado, futuro, aviso: null };
    return {
...
export function refazer(passado, futuro, atual) {
