 ...
 */
import { getVigenciaInfo } from './vigencia.js';
...
function casaTodosOsTermos(termo, campos) {
 ...
 */
export function filtrarContratos(contratos, termo, usuarios) {
    if (!termo.trim())
        return contratos;
 ...
 */
export function camposPorColuna(talento, usuarios) {
    const porId = new Map(usuarios.map((usuario) => [usuario.id, usuario]));
    const pessoa = (id) => {
 ...
 */
export function filtrarTalentos(talentos, termo, usuarios, colunasVisiveis) {
    if (!termo.trim())
        return talentos;
