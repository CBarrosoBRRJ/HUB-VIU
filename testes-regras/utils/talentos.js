...
export function matchesFiltroTalento(talento, filtro) {
    if (filtro === 'todos')
        return true;
    if (filtro === 'pendentes')
        return talento.cadastroPendente === true;
 ...
 */
export function contarTalentos(talentos) {
    return {
...
export function areasDoTalento() {
    const porProjeto = ['pagamento', 'juridico', 'artistico', 'executivo'];
...
export function equipeDaArea(equipes, area) {
 ...
 */
export function candidatosDaArea(equipes, usuarios, area, jaNomeadoId) {
    const equipe = equipeDaArea(equipes, area);
    return usuarios.filter((usuario) => usuario.id === jaNomeadoId || (equipe ? getPapelNaEquipe(equipe, usuario.id) !== null : false));
}
/** Responsáveis de uma área, sempre como lista. */
export function responsaveisDaArea(talento, area) {
 ...
 */
export function alternarResponsavelDaArea(talento, area, usuarioId) {
...
export function limparArea(talento, area) {
    const responsaveis = { ...talento.responsaveis };
    delete responsaveis[area];
    return { ...talento, responsaveis };
}
export function areasDefinidas(talento) {
...
export function nomeadosDoTalento(talento) {
    return [...new Set(Object.values(talento.responsaveis).flat().filter(Boolean))];
}
/**
 ...
 */
export function fichaVazia(talento) {
    const textos = [
    ...
    return (textos.every((valor) => !valor?.trim()) &&
 ...
 */
export function contratosDoTalento(talento, contratos) {
    const nome = normalizar(talento.nome);
    return contratos.filter((contrato) => contrato.talentoId === talento.id ||
...
export function encontrarTalentoPorNome(nome, talentos) {
...
export function nomeEmUso(nome, talentos, ignorarId) {
    const alvo = normalizar(nome);
 ...
 */
export function normalizarNomeDeMarca(nome) {
    return nome
        .trim()
        .toLowerCase()
        .normalize('NFD')
...
export function getOrigemTalento(id) {
 ...
 */
export function fichaDoTalento(oportunidade, talentos) {
