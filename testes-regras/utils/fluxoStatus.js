...
export function destinosPermitidos(atual) {
 ...
 */
export function motivosPermitidosDoDeclinio(origem) {
    if (origem === 'revisao')
        return ['talento'];
    if (origem === 'aguardando_feedback')
        return ['interno', 'mercado', 'talento'];
    return [];
}
...
export function ehEstadoFinal(status) {
...
export function ehEtapaAtiva(status) {
 ...
 */
export function diasParado(oportunidade, referencia = new Date()) {
...
export function deveEncerrarPorTempo(oportunidade, referencia = new Date()) {
    if (!ehEtapaAtiva(oportunidade.status))
        return false;
...
export function diasAteEncerrar(oportunidade, referencia = new Date()) {
    if (!ehEtapaAtiva(oportunidade.status))
        return null;
    const dias = diasParado(oportunidade, referencia);
 ...
 */
export function aplicarEncerramentoAutomatico(oportunidades, referencia = new Date()) {
    const encerradas = [];
    const proximas = oportunidades.map((oportunidade) => {
        if (!deveEncerrarPorTempo(oportunidade, referencia))
            return oportunidade;
 ...
 * edição silenciosa.
 */
export const ETAPAS_CONGELADAS = [
    'revisao',
    'aguardando_feedback',
    /*
 ...
 */
export function ehCongelada(status) {
